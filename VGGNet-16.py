...
def inference_op(input_op, keep_prob):
	...
	conv5_2 = conv_op(conv5_1, name='conv5_2',kh=3, kw=3, n_out=512,dh=1,
					  dw=1, p=p)
	conv5_3 = conv_op(conv5_2, name='conv3_3',kh=3, kw=3, n_out=512,dh=1,
					  dw=1, p=p)				  
	pool5 = mpool_op(conv5_3, name='pool5', kh=2, kw=2, dh=2, dw=2)
	#对第五段卷积网络的输出结果进行扁平化
	shp = pool5.get_shape()
	flattened_shape = shp[1].value*shp[3].value
...
def time_tensorflow_run(session, target, feed, info_string):
	...
	for i in range(num_batches + num_steps_burn_in):
		...
		if i >= num_steps_burn_in:
			if not i % 10:
				print ('%s: step %d, duration = %.4f' %
						(datetime.now(),i-num_steps_burn_in,duration))
			total_duraton += duration
			total_duration_squard += duration*duration
		
	mn = total_duraton / num_batches
